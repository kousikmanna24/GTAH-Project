                    ...
                    <div class="panel-title">
                     ...
                     <div class="panel-body">
                       @if (session('status'))
                        ...
                        @endif
                        <form method="POST" action="{{ route('category.create') }}" id="category_form" novalidate>
                        @csrf
                           <div class="row">
                           ...
                           <div class="row">
                                <div class="form-group col-md-4 col-lg-4">
                                    ...
                                    <select class="col-md-12 btn dropdown-toggle selectpicker btn-option2" id="pid" name="pid" style="text-align:left;">
                                       <option value="0">-- Select Category --</option>
                                       {!! $data['catagories'] !!}
                                    </select>
                                </div>
                                <div class="form-group col-md-4 col-lg-4">
                                    <label for="name" class="form-label">Add Category Path:</label>
                                    <input type="text" class="col-md-12 " id="slug" name="slug" value="" required="required">
                                </div>
                                <div class="form-group col-md-4 col-lg-4">
                                    <label for="name" class="form-label">Menu Category Name:</label>
                                    <input type="text" class="col-md-12 " id="name" name="name" value="" required="required">
                                </div>
                            </div>
                            <div class="row">
                                <input type="submit" class="btn btn-default pull-left" id="addSubCategoryBtn" name="addSubCategoryBtn" value="Add">
                            </div>
                        </form>	
                     </div>
                     ...
                     <div class="panel-body" style="margin-top:50px;">
                        <table id="example0" class="table table-striped table-bordered table-hover dataTable no-footer" role="grid" aria-describedby="example0_info">
                           <thead>
                              <tr role="row">
                                 <th id="default" class="sorting_desc" tabindex="0" aria-controls="example0" rowspan="1" colspan="1" aria-label="Category: activate to sort column ascending" aria-sort="descending" style="width: 131px;">Category</th>
                                 <th id="default" class="sorting" tabindex="0" aria-controls="example0" rowspan="1" colspan="1" aria-label="Menu Category Name: activate to sort column ascending" style="width: 141px;">Menu Category Name</th>
                                 <th class="sorting" tabindex="0" aria-controls="example0" rowspan="1" colspan="1" aria-label="Sub Category: activate to sort column ascending" style="width: 132px;">Sub Category</th>
                                 <th class="sorting" tabindex="0" aria-controls="example0" rowspan="1" colspan="1" aria-label="Menu Sub Category Name: activate to sort column ascending" style="width: 131px;">Menu Sub Category Name</th>
                                 <th class="sorting" tabindex="0" aria-controls="example0" rowspan="1" colspan="1" aria-label="Sub Sub Category: activate to sort column ascending" style="width: 114px;">Sub Sub Category</th>
                                 <th class="sorting" tabindex="0" aria-controls="example0" rowspan="1" colspan="1" aria-label="Menu Sub Sub Category Name: activate to sort column ascending" style="width: 145px;">Menu Sub Sub Category Name</th>
                              </tr>
                           </thead>
                           <tbody>
                           @php $l1cat = App\Models\Category::where('level','=', 1)->get();@endphp

                           @foreach($l1cat as $l1)
                           <tr id="tr1" role="row" class="odd">
                              <td  class="sorting_1">{{ $l1->slug }} 
                                 <a href="{{ route('category.create') }}/edit/{{ $l1->id }}" data-toggle="tooltip" title="" data-original-title="Edit category and category name"><i class="fa fa-pencil-square-o"></i></a>
                              </td>
                              <td>{{ $l1->name }}</td>
                              <td>
                              @php
                              $l2cat = App\Models\Category::where('pid','=', $l1->id)->get();
                              $l2Ids = [];
                              @endphp
                              @foreach($l2cat as $l2)
                                 {{ $l2->slug }}
                                 <a href="{{ route('category.create') }}/edit/{{ $l2->id }}" data-toggle="tooltip" title="" data-original-title="Edit category and category name"><i class="fa fa-pencil-square-o"></i></a>
                                 <br>
                              @endforeach
                              </td>
                              <td>
                              ...
                              <td>
                              @php
                              $l3cat = App\Models\Category::whereIN('pid', $l2Ids)->get();
                              @endphp
                              @foreach($l3cat as $l3)
                                 {{ $l3->slug }}
                                 <a href="{{ route('category.create') }}/edit/{{ $l3->id }}" data-toggle="tooltip" title="" data-original-title="Edit category and category name"><i class="fa fa-pencil-square-o"></i></a>
                                 <br>
                              @endforeach
                              </td>
                              <td>
